...
const find = ({ req, res, next, response }) => {
    return response.build(res, res.advancedResult, 200);
};

const findById = async ({ req, res, next, response, userModel }) => {
    const { id } = req.params;
...
const updateById = async ({ req, res, next, response, userModel }) => {
    const { id } = req.params;
    const user = req.body;
...
const deleteById = async ({ req, res, next, response, userModel }) => {
    const { id } = req.params;
