const register = async ({ req, res, next, userModel, service, config }) => {
    const userData = req.body;
    const user = await userModel.create(userData);
    return service.sendTokenResponse({ user, statusCode: 201, res, config });
};

const login = async ({ req, res, next, userModel, service, config }) => {
...
const forgotPassword = async ({ req, res, next, response, userModel, mailerService, externalMailService, config }) => {
...
const resetPassword = async ({ req, res, next, userModel, service, crypto, config }) => {
